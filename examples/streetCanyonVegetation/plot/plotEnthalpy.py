...
dryRun_data = pandas.read_csv("h_dry_da0.csv")
h_dryRun_da = dryRun_data['h_dry_da']

wetRun_data = pandas.read_csv("h_moist_moist.csv")
h_wetRun_moist = wetRun_data['h_moist_moist']
x,y = dryRun_data['Points:0'], dryRun_data['Points:2']
...
plt.figure(r'Specific Enthalpy - Dry Run')
plt.tricontourf(x, y, h_dryRun_da,cmap=palette['SPECTRAL']['HOT'])
...
plt.figure(r'Specific Enthalpy - Wet Run')
plt.tricontourf(x, y, h_wetRun_moist,cmap=palette['SPECTRAL']['HOT'])
plt.plot([-0.5,0.5,0.5,-.5,-.5],[0.5,0.5,1.5,1.5,0.5],'g',lw=1)
plt.axis('scaled')
plt.axis([-2,6,0,3])
plt.xlabel(r'$x/H$')
