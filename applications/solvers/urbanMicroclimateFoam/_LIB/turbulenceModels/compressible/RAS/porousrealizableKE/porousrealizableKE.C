...
void porousrealizableKE::correct()
{
    ...
    (
     ...
     ==
      ...
      + fvm::Sp(rho_*betaP_*C4_*Cf_*pow(mag(U_),3)/k_,epsilon_) // is it not more stable if positive term is without fvm::Sp (so that it is not in the diagonal)? - ayk
    ...
    (
     ...
     ==
      ...
      + fvm::Sp(rho_*betaP_*Cf_*pow(mag(U_),3)/k_, k_) // is it not more stable if positive term is without fvm::Sp (so that it is not in the diagonal)? - ayk
