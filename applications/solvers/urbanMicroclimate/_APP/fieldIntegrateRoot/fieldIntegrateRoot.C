...
Description
    Calculates the integral value of specific field at the vegetation zone
...
)
{
    if (!done && fieldHeader.headerClassName() == FieldType::typeName)
    {
        ...
        Info<< "    Integral of " << fieldHeader.name()
            ...
            << gSum(mesh.V()*field.internalField()*RAD.internalField()) << " "
            << field.dimensions()*mesh.V().dimensions()*RAD.dimensions()
            << nl;
