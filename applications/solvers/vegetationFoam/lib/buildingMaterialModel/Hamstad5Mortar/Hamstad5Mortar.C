...
:
    ...
    Hamstad5MortarCoeffs_(buildingMaterialProperties.subDict(typeName + "Coeffs")),
    rho_("rho", dimensionSet(1, -3, 0, 0, 0), Hamstad5MortarCoeffs_.lookup("rho")),
    cap_("cap", dimensionSet(0, 2, -2, -1, 0), Hamstad5MortarCoeffs_.lookup("cap"))
    /*Ks_(Hamstad5MortarCoeffs_.lookup("Ks")),
    theta_s_(Hamstad5MortarCoeffs_.lookup("theta_s")),
    theta_r_(Hamstad5MortarCoeffs_.lookup("theta_r")),
    alpha_(Hamstad5MortarCoeffs_.lookup("alpha")),
    beta_(Hamstad5MortarCoeffs_.lookup("beta")),
    gamma_(Hamstad5MortarCoeffs_.lookup("gamma")),
    A_(Hamstad5MortarCoeffs_.lookup("A")),
    Ss_(Hamstad5MortarCoeffs_.lookup("Ss"))*/
{
...
bool Foam::buildingMaterialModels::Hamstad5Mortar::read
(
    const dictionary& buildingMaterialProperties
)
{
    ...
    Hamstad5MortarCoeffs_.lookup("rho") >> rho_.value();
    Hamstad5MortarCoeffs_.lookup("cap") >> cap_.value();

    /*Hamstad5MortarCoeffs_.lookup("Ks") >> Ks_;
    Hamstad5MortarCoeffs_.lookup("theta_s") >> theta_s_;
    Hamstad5MortarCoeffs_.lookup("theta_r") >> theta_r_;
    Hamstad5MortarCoeffs_.lookup("alpha") >> alpha_;
    Hamstad5MortarCoeffs_.lookup("beta") >> beta_;
    Hamstad5MortarCoeffs_.lookup("gamma") >> gamma_;
    Hamstad5MortarCoeffs_.lookup("A") >> A_;
    Hamstad5MortarCoeffs_.lookup("Ss") >> Ss_;*/
...
void Foam::buildingMaterialModels::Hamstad5Mortar::update_w_C_boundary(const volScalarField& pc, volScalarField& w, volScalarField& Crel, label patchi, label patchFacei)
{
    List<scalar> reta; reta.setSize(2); reta[0]=-5.102e-5; reta[1]=-4.082e-7;
    List<scalar> retn; retn.setSize(2); retn[0]=1.5; retn[1]=3.8;
    List<scalar> retm; retm.setSize(2); retm[0]=0.333; retm[1]=0.737;
    ...
    w.boundaryField()[patchi][patchFacei] = w_tmp*700;
    Crel.boundaryField()[patchi][patchFacei] = mag( C_tmp*700 );  
}
...
void Foam::buildingMaterialModels::Hamstad5Mortar::update_Krel_boundary(const volScalarField& pc, const volScalarField& w, volScalarField& Krel, label patchi, label patchFacei)
{
...
void Foam::buildingMaterialModels::Hamstad5Mortar::update_Kv_boundary(const volScalarField& pc, const volScalarField& w, const volScalarField& T, volScalarField& K_v, label patchi, label patchFacei)
{
    scalar rho_l = 1.0e3; 
    scalar R_v = 8.31451*1000/(18.01534); 
    ...
    scalar tmp = 1 - (w.boundaryField()[patchi][patchFacei]/700); 
    scalar delta = 2.61e-5 * tmp/(R_v*T.boundaryField()[patchi][patchFacei]*50*(0.8*tmp*tmp + 0.2)); // Water vapour diffusion coefficient "for brick" [s]
...
void Foam::buildingMaterialModels::Hamstad5Mortar::update_Kpt_boundary(const volScalarField& pc, const volScalarField& w, const volScalarField& T, volScalarField& K_pt, label patchi, label patchFacei)
{
    scalar rho_l = 1.0e3; 
    scalar R_v = 8.31451*1000/(18.01534); 
    scalar L_v = 2.5e6;
...
void Foam::buildingMaterialModels::Hamstad5Mortar::update_lambda_cell(const volScalarField& w, volScalarField& lambda, label& celli)
{

    lambda.internalField()[celli] = 0.6+0.56*w.internalField()[celli]/1000;
}

//- Correct the buildingMaterial lambda (boundary)
void Foam::buildingMaterialModels::Hamstad5Mortar::update_lambda_boundary(const volScalarField& w, volScalarField& lambda, label patchi, label patchFacei)
{

    lambda.boundaryField()[patchi][patchFacei] = 0.6+0.56*w.boundaryField()[patchi][patchFacei]/1000;
}
