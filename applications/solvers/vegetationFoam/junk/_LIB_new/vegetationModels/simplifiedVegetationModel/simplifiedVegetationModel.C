...
double simplifiedVegetationModel::calc_pvsat(double& T)
{
...
void simplifiedVegetationModel::resistance(volScalarField& magU, volScalarField& T, volScalarField& w, volScalarField& Tl_)
{
    ...
    forAll(LAD_, cellI)
    {
        if (LAD_[cellI] > 10*SMALL)
        {
            ...
            pv_[cellI] = p_*w[cellI]/(0.621945+w[cellI]);
            ...
            VPD_[cellI] = pvsat_[cellI] - pv_[cellI];
...
void simplifiedVegetationModel::solve(volVectorField& U, volScalarField& T, volScalarField& w, volScalarField& Tl_)
{
    ...
    for (i=1; i<=maxIter; i++)
    {
        ...
        forAll(LAD_, cellI)
        {
            if (LAD_[cellI] > 10*SMALL)
            {
                ...
                pvsat_[cellI] = calc_pvsat(Tl_[cellI]);
                wsat_[cellI] = 0.621945*(pvsat_[cellI]/(p_-pvsat_[cellI])); // ASHRAE 1, eq.23
...
tmp<volScalarField> simplifiedVegetationModel::Sw()
{
    Sw_ = E_;
