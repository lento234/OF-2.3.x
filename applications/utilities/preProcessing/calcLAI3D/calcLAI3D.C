...
void Foam::calc(const argList& args, const Time& runTime, const fvMesh& mesh)
{
    ...
    if (LADheader.headerOk() && LAIheader.headerOk())
    {
       ...
       dimensionedScalar phi = vegetationProperties.lookup("phi");
       dimensionedScalar beta = vegetationProperties.lookup("beta");
       ...
       vector n1(0,0,1);
       //vector n2(cosPhi, sinPhi, sinBeta);
       vector n2(cosBeta*cosPhi, cosBeta*sinPhi, sinBeta);
       ...
       Info << "pmin: " << transform(Tinv,pmin) << endl;
       Info << "pmax: " << transform(Tinv,pmax) << endl;
       ...
       Info << "max(LAD): " << max(LAD) << endl;
       Info << "min(LAD): " << min(LAD) << endl;

       Info << "max(LADInterp): " << max(LADInterp) << endl;
       Info << "min(LADInterp): " << min(LADInterp) << endl;

       //////////////// Check if tree itself is in the shadow

        //DynamicField<point> start(nx*ny);             // not correct yet
        //DynamicField<point> end(start.size());
        //DynamicList<label> startIndex(start.size());
        //DynamicList<label> endIndex(start.size());

        List<point> start(nx*ny, point::zero);
        List<point> end(start.size());
        ...
        for (int i=0; i < nx; i++)
        {
          for (int j=0; j < ny; j++)
          {
              ...
              start[j*nx+i].x() = pmin.x() + i*dp.x();
              start[j*nx+i].y() = pmin.y() + j*dp.y();
              start[j*nx+i].z() = pmin.z() + k*dp.z();

          }
        }

        // Original coordinate system
        start = transform(Tinv, start);
        ...
        forAll(start, pointI)
        {
            ...
            if (n2.x() > 0.0)
            {
                ix = (pmaxO.x() - start[pointI].x())/n2.x();
            }
            else if (n2.x() < 0.0)
            {
                ix = (pminO.x() - start[pointI].x())/n2.x();
            }
            else {ix = VGREAT;}

            if (n2.y() > 0.0)
            {
                iy = (pmaxO.y() - start[pointI].y())/n2.y();
            }
            else if (n2.y() < 0.0)
            {
                iy = (pminO.y() - start[pointI].y())/n2.y();
            }
            else{iy = VGREAT;}

            if (n2.z() > 0.0)
            {
                iz = (pmaxO.z() - start[pointI].z())/n2.z();
            }
            else if (n2.z() < 0.0)
            {
                iz = (pminO.z() - start[pointI].z())/n2.z();
            }
            else{iz = VGREAT;}

            scalar i = min(ix, min(iy, iz));
            end[pointI] = 0.9999*i*n2 + start[pointI];
            //end[pointI] = i*n2 + start[pointI];
        }
       ...
       for (int i=0; i < nx; i++)
       {
         for (int j=0; j < ny; j++)
         {
           ...
           pIndex = (nx*ny)*k + j*nx + i;
           pointI = nx*j + i;
           pHit = boundaryTree.findLine(start[pointI], end[pointI]);
           if (pHit.hit())
           {
             LADInterp[pIndex] = -1e99;
             Info << "it hit" << endl;
