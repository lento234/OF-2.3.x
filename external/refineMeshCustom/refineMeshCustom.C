...
Description
	...
	Based on the default refineMesh utility	

	Does not refine cells with cell height lower than threshold (lowerLimit).
	
	Aytac Kubilay, Empa Dubendorf, 2013

Usage

    check the sample refineMeshDict file
	modify the value 'lowerLimit'
...
#include "createMesh.H"
    ...
    if (readDict)
    {
        ...
        const word patchName(refineDict.lookup("patch"));
	   scalar lowerLimit = readScalar(refineDict.lookup("lowerLimit"));
	...
	// Cells cut
    	...
    	//Info << "cutCells: " << cutCells << endl << endl;
    	forAll(dOrg[patchID], faceI)
    	{   
        	//Info << "dOrg: " << dOrg[patchID][faceI] << endl;
  		if (dOrg[patchID][faceI] < lowerLimit)
        	{
			cutCells.erase(pp.faceCells()[faceI]);
			//Info << "Erased: " << pp.faceCells()[faceI] << endl; 
        	} 
   	 }
